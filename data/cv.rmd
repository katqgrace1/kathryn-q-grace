...
gt_format <- function(dat){
  dat %>% 
    gt() %>% 
    fmt_markdown(columns = everything()) %>%
    opt_table_lines("none")%>% 
    tab_options(column_labels.hidden = TRUE) %>% 
    cols_width(X1 ~ px(120)) %>% 
    cols_align("left")
}

csv_to_gt <- function(path){
  path %>% 
    here("data",  .) %>% 
...
```{r}
csv_to_gt("appointments.csv")
```

## Affiliations and Visiting Appointments

```{r}
csv_to_gt("affiliations.csv")
```

## Additional Training

```{r}
csv_to_gt("additional_training.csv")
```
...
## Grants and Awards (funding amounts reflect amount received by UMN)
